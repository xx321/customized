...
//		String rootUrl = request.getContextPath() + "/attached/";
		...
		String rootPath = "../upload" + request.getContextPath() + "/attached/";
		String rootUrl = "/upload" + request.getContextPath() + "/attached/";
		...
		if (dirName != null) {
			if (!Arrays.<String> asList(new String[] { "image", "flash", "media", "file" }).contains(dirName)) {
				...
				// return;
				try {
					response.getWriter().write("無效的目錄名稱!");
				} catch (IOException e) {
					e.printStackTrace();
				}
				return m;
		...
		if (path.indexOf("..") >= 0) {
			...
			// return;
			try {
				response.getWriter().write("不允許訪問!");
			} catch (IOException e) {
				e.printStackTrace();
			}
			return m;
		...
		if (!currentPathFile.isDirectory()) {
			...
			// return;
			try {
				response.getWriter().write("目錄不存在!");
			} catch (IOException e) {
				e.printStackTrace();
			}
			return m;
...
//		String saveUrl = request.getContextPath() + "/attached/";
		...
		// 文件保存目录路径
		String savePath = "../upload" + request.getContextPath() + "/attached/";
		// 文件保存目录URL
		String saveUrl = "/upload" + request.getContextPath() + "/attached/";
		...
		if (!uploadDir.canWrite()) {
			m.put("error", 1);
			m.put("message", "上傳目錄沒有寫入的權限!");
			return m;
		}
		
		//關鍵參數，根據前台傳進來的參數，判斷可以寫入的文件(dir = image , flash , media , file) 
		String dirName = request.getParameter("dir"); 
		if (dirName == null) {
			dirName = "image";
		}
				...
				for (FileItem item : items) {
					...
					if (!item.isFormField()) {
						...
						if (item.getSize() > maxSize) {
							m.put("error", 1);
							m.put("message", "上傳文件大小超過限制!(允許最大[" + maxSize + "]位元，您上傳了[" + item.getSize() + "]位元)");
							return m;
						}
						// 检查扩展名
						String fileExt = fileName.substring(fileName.lastIndexOf(".") + 1).toLowerCase();
