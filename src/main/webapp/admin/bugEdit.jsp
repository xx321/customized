		...
		<form id="form" method="post">
			<table class="table table-hover table-condensed">
				<tr>
					...
					<th>BUG名稱</th>
					<td><input name="name" type="text" placeholder="请输入BUG名稱" class="easyui-validatebox span2" data-options="required:true" value="${bug.name}"></td>
				</tr>
				<tr>
					<th>BUG類型</th>
					...
					<c:choose>
						<c:when test="${fn:contains(sessionInfo.resourceList, '/fileController/fileManage')}">
							<th>瀏覽服務器附件</th>
							<td>
								<button type="button" class="btn" onclick="fileManage();">瀏覽服務器</button>
							</td>
						</c:when>
						<c:otherwise>
							<th></th>
