...
<c:if test="${fn:contains(sessionInfo.resourceList, '/orderController/workingOK')}">
...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({         
			url : '${pageContext.request.contextPath}/orderController/workingDataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : false,
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					var str = '';
					if ($.canEdit) {
						str += $.formatString('<button class="btn btn-success" onclick="workingFun(\'{0}\');">確認加工</button>', row.id);
					}
					if ($.canInvalid) {
	...
	function workingFun(id) {
		...
		parent.$.messager.confirm('詢問', '確認加工?', function(b) {
			if (b) {
				parent.$.messager.progress({
					title : '提示',
					text : '數據處理中，請稍後....'
				});
				$.post('${pageContext.request.contextPath}/orderController/workingOK', {
					id : id
				}, function(result) {
					if (result.success) {
...
<body>
	...
	<div id="menu" class="easyui-menu" style="width: 120px; display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/orderController/workingOK')}">
			<div onclick="workingFun();" data-options="iconCls:'pencil_add'">確認加工</div>
		</c:if>
		<c:if test="${fn:contains(sessionInfo.resourceList, '/orderController/invalid')}">
