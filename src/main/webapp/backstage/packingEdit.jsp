...
<script type="text/javascript">
	$(function() {
		...
		$('#form').form({
			url : '${pageContext.request.contextPath}/packingController/edit',
			onSubmit : function() {
			...
			success : function(result) {
				...
				if (result.success) {
					parent.$.modalDialog.openner_dataGrid.datagrid('reload');//因為在option.jsp頁面設定好treeGrid，所以添加成功後，使用這個變數刷新treeGrid。
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;">
		...
		<input name="typeId" type="hidden" value="${option.typeId}" readonly="readonly" />
		<input type="hidden" name="pid" value="${option.pid}" readonly="readonly"/>
			<table class="table table-hover table-condensed">
				...
				<tr>
					<th>備註</th>
					<td colspan="3"><textarea name="remark" rows="" cols="" class="span5">${option.remark}</textarea></td>
				</tr>
			</table>
		</form>
		
		<div id="result" style="margin:10px;width:200px;height:300px;">
			<img id="imgDIV" src="${option.url}" style="width:200px;height:300px;" alt=""/>
		</div> 
	</div>
</div>
