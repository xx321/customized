...
<c:if test="${fn:contains(sessionInfo.resourceList, '/socksController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/socksController/addPage')}">
	<script type="text/javascript">
		$.canAdd = true;
	</script>
</c:if>
...
<script type="text/javascript">
	...
	$(function() {
		treeGrid = $('#treeGrid').treegrid({
			url : '${pageContext.request.contextPath}/optionController/treeGrid?typeId=0',
			idField : 'id',
			treeField : 'name',
			parentField : 'pid',
			fit : true,
	...
	function editFun(id) {
		...
		if (node) {
			parent.$.modalDialog({
				...
				width : 750,
				height : 750,
				href : '${pageContext.request.contextPath}/socksController/editPage?id=' + node.id,
				buttons : [ {
					text : '編輯',
					handler : function() {
	...
	function addFun(rowTypeId,pid) {
		...
		parent.$.modalDialog({
			...
			width : 750,
			height : 750,
			href : '${pageContext.request.contextPath}/socksController/addPage?typeId='+typeId+'&pid='+pid,
			buttons : [ {
				text : '添加',
				handler : function() {
...
<body>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/socksController/addPage')}">
			<a onclick="addFun('null','');" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'pencil_add'">添加襪子分類</a>
		</c:if>
