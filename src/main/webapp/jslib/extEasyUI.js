 ...
 * 
 */
$.fn.panel.defaults.loadingMessage = '加載中....';
$.fn.datagrid.defaults.loadMsg = '加載中....';
 ...
 * @requires jQuery,EasyUI
 * 
 * 创建一个dialog 底下的子dialog
 * 
 * @returns $.modalDialogChildren.handler 这个handler代表弹出的dialog句柄
 * 
 * @returns $.modalDialogChildren.xxx 这个xxx是可以自己定义名称，主要用在弹窗关闭时，刷新某些对象的操作，可以将xxx这个对象预定义好
 */
$.modalDialogChildren = function(options) {
	if ($.modalDialogChildren.handler == undefined) {// 避免重复弹出
		var opts = $.extend({
			title : '',
			width : 840,
			height : 680,
			modal : true,
			onClose : function() {
				$.modalDialogChildren.handler = undefined;
				$(this).dialog('destroy');
			},
			onOpen : function() {
				parent.$.messager.progress({
		...
		opts.modal = true;// 强制此dialog为模式化，无视传递过来的modal参数
		return $.modalDialogChildren.handler = $('<div/>').dialog(opts);
	}
};
 ...
 * 创建一个折扣專用的dialog
 * 
 * @returns $.modalDiscountDialog 这个handler代表弹出的dialog句柄
 * 
 * @returns $.modalDiscountDialog.xxx 这个xxx是可以自己定义名称，主要用在弹窗关闭时，刷新某些对象的操作，可以将xxx这个对象预定义好
 */
$.modalDiscountDialog = function(options) {
	if ($.modalDiscountDialog.handler == undefined) {// 避免重复弹出
		var opts = $.extend({
			title : '',
			width : 840,
			height : 680,
			modal : true,
			onClose : function() {
				//編輯完折扣選項或關閉編輯視窗，連同關閉tab頁面。
	            var tab=index_tabs.tabs("getSelected"); //傳回選單之 Panel 物件
	            if (tab){
	              var index=index_tabs.tabs("getTabIndex",tab);
	              index_tabs.tabs("close",index);
	            }   
				
				$.modalDiscountDialog.handler = undefined;
				$(this).dialog('destroy');
			},
			onOpen : function() {
				parent.$.messager.progress({
		...
		opts.modal = true;// 强制此dialog为模式化，无视传递过来的modal参数
		return $.modalDiscountDialog.handler = $('<div/>').dialog(opts);
	}
};
